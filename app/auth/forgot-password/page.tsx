...
export default async function ForgotPasswordPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <AuthCard title="パスワードを再設定" description="登録メールアドレスへ安全な再設定リンクを送信します。" footer={<Link href="/auth/login">ログインへ戻る</Link>}>
    ...
    <form action={forgotPasswordAction} className="auth-form"><label className="field"><span className="field-label">メールアドレス</span><input name="email" type="email" autoComplete="email" required /></label><button className="button auth-submit" type="submit">再設定メールを送る</button></form>
  </AuthCard>;
}
