...
export default async function HistoryPage() {
  ...
  return (
    <main className="shell">
      ...
      {rows.length ? <div className="history-list">{rows.map(row => <article className="history-row" key={row.id}><div><h2>{row.materialTitle ?? "教材"}</h2><p><Clock3 aria-hidden="true" size={13} /> {new Date(row.startedAt).toLocaleString("ja-JP")}・{row.learnerName}・バージョン {row.versionNumber ?? "—"}</p><StatusBadge tone={row.status === "completed" ? "success" : "warning"}>{row.status === "completed" ? "完了" : "学習中"}</StatusBadge></div><strong className="score-badge">{row.score === null ? "—" : `${row.score}点`}</strong>{row.materialId ? <Link className="button outline" href={`/materials/${row.materialId}`}>教材を見る</Link> : <span />}</article>)}</div> : !loadError && <EmptyState title="学習履歴はまだありません" description="教材を選んで回答を提出すると、日時と得点がここに残ります。" action={<Link className="button" href="/materials">学習を始める</Link>} />}
    </main>
  );
}
