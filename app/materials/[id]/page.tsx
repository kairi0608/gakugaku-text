...
export default async function MaterialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell detail-layout"><div className="no-print"><PageHeader eyebrow={`教材バージョン ${material.version.versionNumber}`} title={material.title} description={`${material.document.metadata.grade}・${material.document.metadata.subject}・${material.document.metadata.unit}`} action={<div className="actions mobile-stack"><Link className="button" href={withExperienceRole(`/learn/${material.id}`, role)}><Play aria-hidden="true" size={17} />{role === "teacher" ? "教材を試す" : "学習を始める"}</Link>{role !== "student" && <Link className="button outline" href={withExperienceRole(`/materials/${material.id}/edit`, role)}><Edit3 aria-hidden="true" size={17} />編集</Link>}</div>} /></div><MaterialRenderer document={material.document} /><MaterialPrintView document={material.document} /></main>;
}
