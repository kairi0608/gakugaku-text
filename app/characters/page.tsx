...
export default async function CharactersPage({ searchParams }: { searchParams: Promise<{ from?: string | string[] }> }) {
  ...
  return (
    <main className="shell">
      <PageHeader eyebrow="学習パートナー" title="キャラクター" description="学習を重ねるとEXPがたまり、キャラクターが成長します。" action={<Link href={withExperienceRole("/characters/new", role)} className="button"><Plus aria-hidden="true" size={18} />新しく作る</Link>} />
      ...
      </div> : !loadError && <EmptyState title="キャラクターがまだいません" description="好きな色やモチーフを選んで、最初の学習パートナーを作りましょう。" action={<Link className="button" href={withExperienceRole("/characters/new", role)}>キャラクターを作る</Link>} />}
