...
export function RoleTopbar({ role, gradeBand }: { role: ExperienceRole; gradeBand?: GradeBand | null }) {
  ...
  return <header className="app-topbar"><div className="mobile-brand" aria-label="ガクガクAIシステム"><span className="brand-mark"><BookOpenCheck aria-hidden="true" size={20} /></span><strong>ガクガクAIシステム</strong></div><p className="topbar-title">{title}</p><span className="topbar-caption">{experienceRoleDescriptions[role]}</span><form action={logoutAction}><button className="topbar-logout" type="submit">ログアウト</button></form></header>;
}
