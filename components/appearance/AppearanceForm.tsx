...
export function AppearanceForm({ assets, initial }: { assets: Asset[]; initial: Settings }) {
  ...
  async function save() {
    ...
    try {
      await requestJson("/api/settings/appearance", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ assetId, fit, position, overlayStrength, blur }) }, "背景設定を保存できませんでした。");
      setMessage("背景設定を保存しました。再読み込みすると全画面に反映されます。"); router.refresh();
    } catch (reason) {
      setMessage(userErrorMessage(reason, "背景設定を保存できませんでした。"));
    } finally {
      setBusy(false);
    }
  }
...
export function BackgroundGenerator() {
  ...
  async function generate(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setMessage(""); const form = new FormData(event.currentTarget); try { await requestJson("/api/backgrounds/generate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(Object.fromEntries(form)) }, "背景を生成できませんでした。"); setMessage("背景を生成しました。保存済み背景から選択できます。"); router.refresh(); } catch (reason) { setMessage(userErrorMessage(reason, "背景を生成できませんでした。")); } finally { setBusy(false); } }
...
export function BackgroundUploader() {
  const router = useRouter(); const [busy, setBusy] = useState(false); const [message, setMessage] = useState("");
  async function upload(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setMessage(""); const formElement = event.currentTarget; try { await requestJson("/api/backgrounds/upload", { method: "POST", body: new FormData(formElement) }, "背景をアップロードできませんでした。"); formElement.reset(); setMessage("背景を保存しました。"); router.refresh(); } catch (reason) { setMessage(userErrorMessage(reason, "背景をアップロードできませんでした。")); } finally { setBusy(false); } }
