...
export function LearningFeedback({ document, evaluations, aggregate, feedbackStatus }: { document: MaterialDocument; evaluations: EvaluationWithQuestion[]; aggregate?: AggregateFeedback; feedbackStatus: "not-required" | "pending" | "complete" | "failed" }) {
  ...
  return <div className="learning-feedback">
    {aggregate && <section className="aggregate-feedback"><div className="feedback-heading"><CircleCheckBig aria-hidden="true" size={22} /><h2>学習全体のフィードバック</h2></div><p>{aggregate.summary}</p><details><summary>詳しい振り返りを見る</summary><div className="feedback-columns"><div><h3>できていたこと</h3><ul>{aggregate.strengths.map(item => <li key={item}>{item}</li>)}</ul></div><div><h3>次に取り組むこと</h3><ul>{aggregate.improvements.map(item => <li key={item}>{item}</li>)}</ul></div></div><p><strong>おすすめ:</strong> {aggregate.recommendedNextSteps.join("・")}</p><p>{aggregate.encouragement}</p></details></section>}
    {evaluations.length > 0 && <details className="question-feedback-details"><summary>問題ごとの確認を見る</summary>{evaluations.map(item => { const question = document.questions.find(candidate => candidate.id === item.questionId); return <section className="evaluation-card" key={item.questionId}><div className="feedback-heading"><StatusIcon verdict={item.verdict} /><strong>問題 {question?.order ?? ""}・{verdictLabel(item.verdict)}（{item.score}点）</strong></div><p>{item.goodPoint}</p><p>{item.improvement}</p>{document.feedbackPolicy.allowHints && item.hint && <p className="hint">ヒント: {item.hint}</p>}{document.feedbackPolicy.revealAnswer && item.modelAnswer && <p>回答例: {item.modelAnswer}</p>}</section>; })}</details>}
  </div>;
}
