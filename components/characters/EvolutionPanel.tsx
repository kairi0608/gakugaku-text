...
export function EvolutionPanel({ characterId, exp, assets }: { characterId: string; exp: number; assets: Asset[] }) {
  ...
  async function generate(stage: Stage) {
    ...
    const response = await fetch("/api/characters/images", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ characterId, stage, request: "これまでの特徴を大切にした、親しみやすい成長姿" }) });
    const result = await response.json(); setBusy(false);
    if (!response.ok) { setMessage(result.error ?? "進化画像を生成できませんでした。"); return; }
  ...
  async function apply(asset: Asset) {
    setBusy(true); setMessage("");
    const response = await fetch(`/api/characters/${characterId}/evolution/apply`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ characterAssetId: asset.id }) });
    const result = await response.json(); setBusy(false);
    if (!response.ok) { setMessage(result.error ?? "画像を適用できませんでした。"); return; }
