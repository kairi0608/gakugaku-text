...
9. Productionでは **Authentication → SMTP Settings** でCustom SMTPを設定し、そのSMTP事業者側の配信ログ・抑止リスト・ドメイン認証を確認

Auth Logsで特に確認するerror code:

- `over_email_send_rate_limit`: 送信回数制限。時間を置いて再試行する
- `email_address_not_authorized`: 現在の送信設定では宛先が許可されていない
- `unexpected_failure`: SMTPまたはAuth内部処理を含む予期しない失敗。SMTP Provider側のログも確認する
...
## 7. Custom SMTP（Productionの必須確認）

メール確認を使うProduction環境では、Supabaseの **Authentication → SMTP Settings** でCustom SMTPを設定してください。Supabase built-in email providerだけに依存した状態は、送信先や回数に制約があり、本番のメール配送確認を完了したことにはできません。

Sender domain、From address、SMTP host、port、username、passwordはSupabase Dashboard側へ設定します。値そのものを `.env.example`、ソースコード、GitHubへ保存しないでください。SPF・DKIMなどの送信ドメイン認証と、SMTP事業者側の配信ログも確認します。

アプリの `/admin/system` はApplication URL、Callback URL、Supabase公開接続の設定有無だけを表示します。SMTP設定や実際の配送成功は推測しません。配送結果は **Authentication → Logs** とSMTP Providerで確認してください。
